...
<template>
    <Card>
        <CardHeader>
            ...
            >
                <div>
                    <CardTitle>All Acceptance & Inspection Reports</CardTitle>
                    <CardDescription>
                        Generated reports sourced from Purchase Orders
                    </CardDescription>
                </div>
        ...
        <CardContent>
            <div class="relative w-full overflow-auto">
                <table class="w-full caption-bottom text-sm">
                    ...
                    <tbody class="[&_tr:last-child]:border-0">
                        <tr v-if="!acceptanceInspections?.data?.length">
                            <td
                                colspan="5"
                        ...
                        <tr
                            v-for="report in acceptanceInspections.data"
                            :key="report.id"
                            class="border-b transition-colors hover:bg-muted/50"
                        >
                            <td class="p-4 align-middle">
                                <div class="font-medium">
                                    {{ report.purchase_order?.po_no || "—" }}
                                </div>
                                <div
                                    class="text-xs text-muted-foreground truncate max-w-[320px]"
                                >
                                    {{
                                        report.purchase_order?.noa
                                            ?.bac_resolution?.aoq
                            ...
                            <td class="p-4 align-middle text-center">
                                {{
                                    formatDate(report.acceptance_date_received)
                                }}
                            </td>
                            <td class="p-4 align-middle text-center">
                            ...
                            <td class="p-4 align-middle">
                                {{
                                    report.purchase_order?.noa?.bac_resolution
                            ...
                            <td class="p-4 align-middle text-right">
                                ...
                                >
                                    ...
                                    <Button
                                        ...
                                        @click="$emit('delete-click', report)"
                                    >
                                        <Icon
                                            icon="lucide:trash-2"
                                            class="h-4 w-4"
            ...
            <div class="mt-4 flex items-center justify-between border-t pt-4">
                <div class="text-sm text-muted-foreground">
                    Showing {{ acceptanceInspections.from }} to
                    {{ acceptanceInspections.to }} of
                    {{ acceptanceInspections.total }} entries
                </div>
                <div class="flex items-center gap-1">
                    <template
                        v-for="(link, index) in acceptanceInspections.links"
                        :key="index"
                    >
                        <Link
                            v-if="link.url"
